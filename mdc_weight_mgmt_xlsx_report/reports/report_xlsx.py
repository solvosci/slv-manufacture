...
class MdcWeightXlsxReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, weight_records):
        ...
        for equipment in equipments:
            ...
            worksheet.write(f'B{row}', _('Nominal Weight (%s)',lines.weight_uom.name), header_format)
            worksheet.write(f'C{row}', _('Declared Weight (%s)',lines.weight_uom.name), header_format)
            ...
            worksheet.write(f'H{row}', _('Total Units/min'), header_format)
            worksheet.write(f'I{row}', _('Units Accepted'), header_format)
            worksheet.write(f'J{row}', _('Units Accepted(%)'), header_format)
            worksheet.write(f'K{row}', _('Total Accepted Weight(%s)',lines.weight_total_uom.name), header_format)
            worksheet.write(f'L{row}', _('Total Declared Weight(%s)',lines.weight_total_uom.name), header_format)
            worksheet.write(f'M{row}', _('Average Weight Accepted(%s)',lines.weight_uom.name), header_format)
            worksheet.write(f'N{row}', _('Overweight (%)'), header_format)
            worksheet.write(f'O{row}', _('Units Rejected Overweight'), header_format)
            worksheet.write(f'P{row}', _('Units Rejected Underweight'), header_format)
            worksheet.write(f'Q{row}', _('Total Units Rejected'), header_format)
            worksheet.write(f'R{row}', _('Units Rejected (%)'), header_format)
            row += 1
