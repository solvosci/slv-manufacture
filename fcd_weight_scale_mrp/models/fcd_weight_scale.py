...
class FCDWeightScale(models.Model):
    _name = 'fcd.weight.scale'
    _description = 'Scale'

    name = fields.Char('Name',required=True,copy=False,default=_('New scale'))
    tcp_address_ip = fields.Char('IP Address',required=True)
    tcp_address_port = fields.Integer('IP Port',required=True)
    timeout_secs = fields.Float('Timeout (s)',required=True,default=5)
    scale_protocol = fields.Selection([('$', '$ Protocol')],string='Weighing protocol',required=True,default='$')
    weight_uom_id = fields.Many2one('uom.uom',string='Weight Unit',domain="[('category_id','=',2)]")
    last_weight_value = fields.Float('Last weight value',readonly=True,copy=False)
    last_weight_uom_id = fields.Many2one('uom.uom',string='Last weight Unit',readonly=True)
    last_weight_stability = fields.Selection([('stable', 'Stable'), ('unstable', 'Unstable'), ('unknown', 'Unknown')],string='Last weight stability',readonly=True,copy=False)
    last_weight_datetime = fields.Datetime('Last weight date',readonly=True,copy=False)
